...
use ru_command_protocol::{
    ...
    AgentRegistration, PbAgentError, PbAgentPayloadEnvelope, PbMqttConnAck, PbMqttConnect,
    PbMqttFrame, PbMqttPingReq, PbMqttPingResp, PbMqttPublish, PbMqttSubAck, PbTaskCancel,
...
pub(crate) async fn handle_agent_socket(
    state: Arc<AppState>,
    ...
    if connect.client_id != path_agent_id {
        let _ = socket
            .send(WsMessage::Binary(
    ...
    if let Err(message) = state
        .auth
        .verify_agent_token(&path_agent_id, &connect.auth_token)
    {
        let _ = socket
            .send(WsMessage::Binary(
    ...
    state.insert_session(path_agent_id.clone(), signal_tx);
    ...
    loop {
        tokio::select! {
            _ = heartbeat.tick() => {
                if handle_in_flight_timeout(&state, &path_agent_id, &mut in_flight_task).await.is_err() {
                    break;
                }
                if send_mqtt_frame(
                    &mut socket,
    ...
    state.remove_session(&path_agent_id);
    let _ = state
        .db
        .requeue_incomplete_tasks(&path_agent_id, "agent session closed");
    let _ = socket.close().await;
}
...
async fn handle_publish(
    ...
    if publish.topic == agent_hello_topic(agent_id) {
        let Some(AgentPayloadBody::ClientHello(hello)) = payload.body else {
            publish_error(
                socket,
                agent_id,
                "hello topic requires client hello payload",
            )
            .await?;
            return Ok(());
        };
        if hello.agent_id != agent_id {
            publish_error(socket, agent_id, "agent_id mismatch in hello payload").await?;
            return Ok(());
        }

        state
            .db
            .requeue_incomplete_tasks(agent_id, "agent re-registered")
            .map_err(to_io_error)?;
        state
            .db
            .register_agent(AgentRegistration::from(hello))
            .map_err(to_io_error)?;
        *registered = true;
        try_dispatch_next_task(
            state,
    ...
    if publish.topic == agent_ack_topic(agent_id) {
        let Some(AgentPayloadBody::TaskAck(ack)) = payload.body else {
            publish_error(socket, agent_id, "ack topic requires task ack payload").await?;
            return Ok(());
        };
        ...
        if current_task.task_id != ack.task_id {
            publish_error(socket, agent_id, "unexpected task ack id").await?;
            return Ok(());
        }

        state
            .db
            .acknowledge_task(ack.task_id, agent_id)
            .map_err(to_io_error)?;
        return Ok(());
    }

    if publish.topic == agent_result_topic(agent_id) {
        let Some(AgentPayloadBody::TaskResult(result)) = payload.body else {
            publish_error(
                socket,
                agent_id,
                "result topic requires task result payload",
            )
            .await?;
            return Ok(());
        };
        let Some(current_task) = in_flight_task.as_ref() else {
            if task_is_canceled(state, result.task_id).map_err(to_io_error)? {
                return Ok(());
            }
            publish_error(
                socket,
        ...
        if current_task.task_id != result.task_id {
            ...
            publish_error(socket, agent_id, "unexpected task result id").await?;
            return Ok(());
        }
...
async fn try_dispatch_next_task(
    ...
    if !registered
        ...
        || !subscriptions.contains(&agent_task_topic(agent_id))
    {
        return Ok(());
    }

    let task = state.db.claim_task(agent_id).map_err(to_io_error)?;
    let ClaimTaskOutcome::Claimed(task) = task else {
        return Ok(());
    };

    let payload = PbAgentPayloadEnvelope {
        body: Some(AgentPayloadBody::TaskAssignment((&task).into())),
    };
    send_mqtt_frame(
        socket,
        PbMqttFrame {
            body: Some(MqttFrameBody::Publish(PbMqttPublish {
                topic: agent_task_topic(agent_id),
...
async fn handle_cancel_signal(
    ...
    if let Some(task) = in_flight_task.as_ref() {
        if task.task_id == task_id {
            let payload = PbAgentPayloadEnvelope {
                body: Some(AgentPayloadBody::TaskCancel(PbTaskCancel {
                    task_id,
                    reason,
                })),
            };
            send_mqtt_frame(
                ...
                PbMqttFrame {
                    body: Some(MqttFrameBody::Publish(PbMqttPublish {
                        topic: agent_control_topic(agent_id),
                        payload: payload.encode_message(),
                    })),
                },
            )
...
fn task_is_canceled(state: &Arc<AppState>, task_id: u64) -> Result<bool, crate::db::DbError> {
    Ok(matches!(
        state.db.get_task(task_id)?,
        Some(task) if task.status == ru_command_protocol::TaskStatus::Canceled
    ))
}
...
async fn handle_in_flight_timeout(
    ...
    let Some(task) = in_flight_task.as_ref() else {
        return Ok(());
    };
    ...
    state
        .db
        .retry_task(task.task_id, agent_id, "task timeout expired")
        .map_err(to_io_error)?;
    *in_flight_task = None;
    Ok(())
}
...
async fn publish_error(
    ...
    let payload = PbAgentPayloadEnvelope {
        body: Some(AgentPayloadBody::Error(PbAgentError {
