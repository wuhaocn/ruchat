...
export RU_SERVER_DB_PATH="${RU_SERVER_DB_PATH:-./ruchat.db}"
...
if [[ "${MODE}" == "split" ]]; then
  ...
  echo "starting ru-command-server in split mode: http=${RU_SERVER_HTTP_BIND} ws=${RU_SERVER_WS_BIND}"
else
  ...
  echo "starting ru-command-server in single mode: bind=${RU_SERVER_HTTP_BIND}"
fi
