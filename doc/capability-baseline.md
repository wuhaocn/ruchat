...
- 服务端返回该 node 对应的 WebSocket 地址和心跳周期
...
- 接收 node WebSocket 连接
- 维护在线 session
- 创建、派发、取消、回收任务
- 提供后台控制台
- 持久化 node 和任务数据到 SQLite
...
- 客户端收到后应终止当前本地子进程
- 服务端在旧任务退出或晚到结果被忽略前，不应继续向同一 node 并发派发下一条任务
...
- 单个 node session 同一时刻只允许一个 in-flight task
- 当前协议实现默认单任务串行执行
...
- 服务端先把任务回退到可重试状态
- 同时向客户端发送取消信号
- 不会在旧进程还未退出时立刻继续向该 node 并发派发新任务
